...
def monitor_stream(stream_id, stream_url):
    """Monitor a video stream and use DeepFace's find() to detect target individuals"""
    with app.app_context():
        logger.info(
            f"Starting monitoring for stream {stream_id}: {stream_url}")
        ...
        try:
            ...
            while stream.active:
                ...
                if not ret:
                    logger.warning(
                        f"Failed to read frame from stream {stream_id}")
                    break

                frame_count += 1

                # Process every 15 frames to reduce load
                if frame_count % app.config["RECOGNITION_FRAME_RATE"] == 0:
                    try:
                        results = DeepFace.find(
                            img_path=frame,
                            db_path=app.config["TARGET_DIR"],
                            model_name=app.config["RECOGNITION_MODEL_NAME"],
                            distance_metric=app.config["RECOGNITION_DISTANCE_METRIC"],
                            detector_backend=app.config["RECOGNITION_DETECTOR_BACKEND"],
